...
**以上部分参考：https://github.com/SirlyDreamer/domjudge-compose/wiki/如何通过docker-compose便捷的部署DOMjudge**.
...
```txt
{"id":"gold-medal","parameters":{"numTeams":"72"}}
{"id":"first-to-solve-*"}
{"id":"honors-mention","parameters":{"percentileTop":"50","percentileBottom":"100"}}
```

其中 gold-medal 的参数是需要单步操作滚榜的范围，honors-mention 是快速滚过去的比例。把 medal 的展示方式改为 Ignore，然后给需要单步操作滚榜的范围加上 rank 奖，然后手动设置金银铜奖。

执行：
```bash
ICPC_FONT="Microsoft Yahei" ./resolver.sh cdp --display_name "{org.formal_name} --{team.display_name}"
```
即可。
